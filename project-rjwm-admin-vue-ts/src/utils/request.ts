...
service.interceptors.response.use(
  (response: any) => {
    ...
    if (response.config.__pendingKey) {
      removePending(response.config.__pendingKey);
    }
  ...
  (error: any) => {
    // A duplicate cancellation has no config and must not remove the original request.
    ...
    //请求响应中的config的url会带上代理的api需要去掉
    if (error.config && error.config.__pendingKey) {
      removePending(error.config.__pendingKey);
    }
