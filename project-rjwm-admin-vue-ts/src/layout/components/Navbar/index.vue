...
export default class extends Vue {
  ...
  webSocket() {
    ...
    const socketUrl = buildSocketUrl(process.env.VUE_APP_SOCKET_URL, window.location.href, clientId)
    if (typeof WebSocket == 'undefined') {
      ...
      this.websocket.onopen = function () {
        that.websocket.send(JSON.stringify({ type: 'authenticate', token: UserModule.token }))
      }
      // 监听socket消息接收
      this.websocket.onmessage = function (msg) {
        const jsonMsg = JSON.parse(msg.data)
        if (jsonMsg.type !== 1 && jsonMsg.type !== 2) return
        // 转换为json对象
